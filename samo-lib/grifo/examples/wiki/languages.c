...
const unsigned char *get_hiragana(const unsigned char *in_str, int len, int *used_len)
{
	...
	if (bFound) // find the first hiragana_mapping entry with the same starting character as in_str
	{
		...
		{
			if (len >= ustrlen(english_hiragana_mapping[i].english) && !ustrncmp(in_str, english_hiragana_mapping[i].english, ustrlen(english_hiragana_mapping[i].english)))
			{
				*used_len = ustrlen(english_hiragana_mapping[i].english);
				pReturn = (const unsigned char *)english_hiragana_mapping[i].hiragana;
			}
		}
	}
...
const unsigned char *get_english(const unsigned char *in_str, int len, int *used_len)
{
	...
	get_first_utf8_char(first_utf8_char, in_str, ustrlen(in_str));
	len_first_char = ustrlen(first_utf8_char);
	*used_len = len_first_char;
	while (!bFound && iStart <= iEnd)
	{
		iMiddle = (iStart + iEnd) / 2;
		cmp = ustrncmp(first_utf8_char, zh_jp_english_mapping[iMiddle].hiragana, len_first_char);
		if (!cmp)
			bFound = 1;
		else if (cmp > 0)
		{
	...
	if (bFound) // find the first hiragana_mapping entry with the same starting character as in_str
	{
		while (iMiddle > 0 && !ustrncmp(first_utf8_char, zh_jp_english_mapping[iMiddle - 1].hiragana, len_first_char))
			iMiddle--;
			...
			!ustrncmp(first_utf8_char, zh_jp_english_mapping[i].hiragana, len_first_char); i++)
		{
			...
			{
				*used_len = ustrlen(zh_jp_english_mapping[i].hiragana);
				pReturn = (const unsigned char *)zh_jp_english_mapping[i].english;
			}
		}
	}
...
void hiragana_romaji_conversion(unsigned char *search_string_per_language, int *search_str_per_language_len)
{
	...
	unsigned char temp_hiragana[MAX_TITLE_SEARCH * 3];
	int temp_hiragana_len = 0;
	int i;
	const unsigned char *pHiragana = NULL;
	int used_len;
	int iFirstRomaji = -1;
	...
	if (*search_str_per_language_len > 0)
	{
		...
		while (i < *search_str_per_language_len)
		{
			...
			{
				const unsigned char *pFull;

				if (!(search_string_per_language[i] & 0x80) && (pFull = half_alphabet_to_full(search_string_per_language[i])))
				{
					ustrcpy(&temp_hiragana[temp_hiragana_len], pFull);
					temp_hiragana_len += ustrlen(pFull);
					i++;
				}
				else
			...
			else
			{
				...
				if (pHiragana)
				{
					...
					memcpy(&temp_hiragana[temp_hiragana_len], pHiragana, ustrlen(pHiragana));
					temp_hiragana_len += ustrlen(pHiragana);
					i += used_len;
				}
				else
				{
...
int replace_japanese_sonant(unsigned char *search_string_per_language, int *search_str_per_language_len, unsigned char *search_string, int *search_str_len)
{
	...
	if (*search_str_per_language_len > 0)
	{
		...
		{
			iMiddle = (iStart + iEnd) / 2;
			cmp = ustrcmp(last_utf8_char, sonant_conversion[iMiddle].origin);
			if (!cmp)
				bFound = 1;
			else if (cmp > 0)
			{
		...
		if (bFound)
		{
			memcpy(&search_string_per_language[*search_str_per_language_len - ustrlen(last_utf8_char)], sonant_conversion[iMiddle].after_conversion, ustrlen(sonant_conversion[iMiddle].after_conversion));
			*search_str_per_language_len += ustrlen(last_utf8_char) - ustrlen(sonant_conversion[iMiddle].after_conversion);
			search_string_per_language[*search_str_per_language_len] = '\0';
...
int replace_hiragana_backward(unsigned char *search_string_per_language, int *search_str_per_language_len, unsigned char *search_string, int *search_str_len)
{
	...
	if (*search_str_per_language_len > 0)
	{
		...
		{
			iMiddle = (iStart + iEnd) / 2;
			cmp = ustrcmp(last_utf8_char, backward_key_sequence[iMiddle].origin);
			if (!cmp)
				bFound = 1;
			else if (cmp > 0)
			{
		...
		if (bFound)
		{
			memcpy(&search_string_per_language[*search_str_per_language_len - ustrlen(last_utf8_char)], backward_key_sequence[iMiddle].after_conversion, ustrlen(backward_key_sequence[iMiddle].after_conversion));
			*search_str_per_language_len += ustrlen(backward_key_sequence[iMiddle].after_conversion) - ustrlen(last_utf8_char);
			search_string_per_language[*search_str_per_language_len] = '\0';
...
int zh_jp_to_english(unsigned char *sEnglish, int maxLenEnglish, unsigned char *sHiragana, int *lenHiragana)
{
	...
	while (i < *lenHiragana && lenEnglish < maxLenEnglish - 1)
	{
		...
		if (pEnglish)
		{
			if (lenEnglish < maxLenEnglish - ustrlen(pEnglish))
			{
				memcpy(&sEnglish[lenEnglish], pEnglish, ustrlen(pEnglish));
				lenEnglish += ustrlen(pEnglish);
				i += used_len;
			}
			else
			{
...
void alphabet_to_korean_jamo(unsigned char *jamo, unsigned char eng)
{
	int bFound = 0;
	int iStart = 0;
