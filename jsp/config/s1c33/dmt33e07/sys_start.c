...
static	void	hardware_init_hook(void)
{
	volatile unsigned long	*pulBcuBase;
	unsigned long		ulTemp;

	// Set Area8(SRAM) and Area10(NOR Flash)'s wait
	pulBcuBase = (volatile unsigned long *)S1C33_BCU_BASE;
	pulBcuBase[0] = 0x00000001;					// Divide clock by 2

	ulTemp =  pulBcuBase[1];
	ulTemp &= 0xf0f0ffff;
	ulTemp |= 0x02020000;						// Area8  wait = 2
	pulBcuBase[1] = ulTemp;						// Area10 wait = 2
	
	ulTemp =  pulBcuBase[2];
	ulTemp &= 0xfffffcff;
	ulTemp |= 0x00000100;
	pulBcuBase[2] = ulTemp;						// Area8 bus width = 16bit
	
	pulBcuBase[3] = 0x00000010;					// Area8 Access type = #BSL
