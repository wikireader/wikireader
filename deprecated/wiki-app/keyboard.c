	...
	if (!bOverwrite)
	{
		...
		for (i = 0; i < KEY_BUBBLE_STEM_HEIGHT; i++) // drawn from the bottom of the stem
		{
			...
			else if (1 <= i && i <= 2)
			{
				...
				lcd_set_framebuffer_pixel(bubble_stem_start_x + bubble_stem_width - 4 + i, bubble_stem_start_y + (KEY_BUBBLE_STEM_HEIGHT - 1 - i));
			...
			else
			{
				...
				lcd_set_framebuffer_pixel(bubble_stem_start_x + bubble_stem_width - 1, bubble_stem_start_y + (KEY_BUBBLE_STEM_HEIGHT - 1 - i));
...
void keyboard_process_key_invert(struct keyboard_key *key, bool bResetDelay)
{
	...
	else
	{
		if (kb_mode == KEYBOARD_CHAR_KO && is_korean_special_key(key))
		{
			...
			key_bubble_start_time = get_time_ticks();
		}
		else
		{
...
int keyboard_korean_special_key(void)
{
	...
	{
		if (time_diff(get_time_ticks(), key_bubble_start_time) > seconds_to_ticks(KOREAN_SPECIAL_KEY_CHANGE_TIME))
		{
			char s[2];
