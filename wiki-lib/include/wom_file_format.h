...
// On our factory SD card, we set a FAT32 cluster size of 4KiB to format the 8GiB card with 1 partition.
// tbd: the FAT cluster size needs more research, for example a 2GB file would have 2 MB of cluster data, i.e.
//      seeking to the end of the file would require reading all this data (=15+ seconds). Maybe we need to
//      go to higher cluster sizes, for example 16 KiB.
...
typedef struct wom_header
{
	...
	uint32_t num_articles, num_bitmaps;
	uint32_t index_first_page, index_num_pages;
	uint32_t bitmaps_first_page, bitmaps_num_pages;
...
typedef struct wom_page_element
{
	...
	// If offset == 0xFFFFFFFF, it means this element is only used to advance the y coordinate. Otherwise it's empty.
	// If bit 31 is set, the element is all or part of a link.
	// If bit 30 is not set, offset is an offset into the bitmaps, pointing to a wom_bitmap_t.
	uint32_t offset;
